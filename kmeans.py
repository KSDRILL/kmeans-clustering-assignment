...
# ============================================
# DATA ANALYSIS - ADD THIS AT THE VERY END
# ============================================
print("\n" + "="*60)
print(f"K-MEANS CLUSTERING ANALYSIS (k={k})")
print("="*60)
...
for cluster in sorted(df["Cluster"].unique()):
    ...
    print(f"\nCluster {cluster}:")
    print(f"  Nodes: {len(cluster_data)}")
    print(f"  Avg Energy: {cluster_data['Energy'].mean():.1f}")
    print(f"  Min Energy: {cluster_data['Energy'].min()}")
    print(f"  Max Energy: {cluster_data['Energy'].max()}")
    print(f"  Energy Std: {cluster_data['Energy'].std():.1f}")

print("\n" + "-"*60)
print("CRITICAL NODES (Lowest Energy in Each Cluster)")
print("-"*60)

for cluster in sorted(df["Cluster"].unique()):
    cluster_data = df[df["Cluster"] == cluster]
    lowest_node = cluster_data.loc[cluster_data['Energy'].idxmin()]
    print(f"Cluster {cluster}: Node {lowest_node['Node']} | Energy: {lowest_node['Energy']} | Position: ({lowest_node['X']:.1f}, {lowest_node['Y']:.1f})")

print("\n" + "-"*60)
print("GLOBAL STATISTICS")
print("-"*60)
print(f"Overall Avg Energy: {df['Energy'].mean():.1f}")
print(f"Overall Min Energy: {df['Energy'].min()}")
print(f"Overall Max Energy: {df['Energy'].max()}")
print(f"Lowest Energy Node: {df.loc[df['Energy'].idxmin(), 'Node']} (Energy: {df['Energy'].min()})")
print(f"Highest Energy Node: {df.loc[df['Energy'].idxmax(), 'Node']} (Energy: {df['Energy'].max()})")